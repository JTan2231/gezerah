...
export async function createDisposableDatabase(): Promise<DisposableDatabase> {
  const baseURL =
    process.env.SCRYER_TEST_DATABASE_URL ??
    process.env.SCRYER_E2E_ADMIN_DATABASE_URL ??
    process.env.SCRYER_DATABASE_URL ??
    "postgres://localhost:5432/postgres?sslmode=disable";
  const databaseName = `scryer_e2e_${Date.now()}_${randomBytes(4).toString("hex")}`;
