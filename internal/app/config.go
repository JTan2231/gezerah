...
func ValidateConfig(config Config) error {
	...
	if origin != "" && !secure && !loopbackListenAddress(config.Addr) {
		return fmt.Errorf("WROUGHT_ADDR must bind to loopback when WROUGHT_PUBLIC_ORIGIN uses HTTP")
	}
	return nil
}
...
func parsePublicOrigin(value string) (string, bool, error) {
	...
	if err != nil {
		return "", false, fmt.Errorf("WROUGHT_PUBLIC_ORIGIN must be a valid http(s) origin: %w", err)
	}
	...
	if parsed.Host == "" || (scheme != "http" && scheme != "https") ||
		...
		return "", false, fmt.Errorf("WROUGHT_PUBLIC_ORIGIN must be an http(s) origin without credentials, a path, query, or fragment")
	}
	hostname := strings.ToLower(parsed.Hostname())
	if hostname == "" || strings.Contains(hostname, "%") {
		return "", false, fmt.Errorf("WROUGHT_PUBLIC_ORIGIN host is invalid")
	}
	if scheme == "http" && !loopbackHostname(hostname) {
		return "", false, fmt.Errorf("WROUGHT_PUBLIC_ORIGIN may use HTTP only with a loopback host; use HTTPS for non-local origins")
	}
	for _, character := range hostname {
		if character > 127 {
			return "", false, fmt.Errorf("WROUGHT_PUBLIC_ORIGIN host must use ASCII or an IDNA-encoded name")
		}
	}
...
func LoadConfig() Config {
	return Config{
		Addr: firstNonEmpty(
			os.Getenv("WROUGHT_ADDR"),
			addrFromPort(os.Getenv("PORT")),
			":8080",
		),
		DatabaseURL: firstNonEmpty(
			os.Getenv("WROUGHT_DATABASE_URL"),
			os.Getenv("DATABASE_URL"),
			"postgres://localhost:5432/wrought?sslmode=disable",
		),
		PublicOrigin:  strings.TrimSpace(os.Getenv("WROUGHT_PUBLIC_ORIGIN")),
		OpenAIAPIKey:  strings.TrimSpace(os.Getenv("OPENAI_API_KEY")),
		OpenAIBaseURL: strings.TrimSpace(os.Getenv("WROUGHT_OPENAI_BASE_URL")),
		LogLevel:      parseLogLevel(os.Getenv("WROUGHT_LOG_LEVEL")),
	}
}
