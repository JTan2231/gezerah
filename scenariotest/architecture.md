...
authoritative run and are retained only when `DND_E2E_DIAGNOSTICS=1` is set.
...
`DND_E2E_APP_BINARY`. `test/src/appServer.ts` builds only as a safe fallback for
...
Root E2E sets `DND_E2E_REQUIRE_COMPLETE_COVERAGE=1`, so any required scenario
