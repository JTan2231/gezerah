...
| `RST-002` | **Save direct setup input state.** An owner/editor changes the complete logical input map in Build using current state/rules revisions. Inputs display new intrinsic/effective values; calculated mechanics remain read-only and no live receipt is fabricated.      | P0       | `UI`, `HTTP`, `DB`; `happy`, `rules`, `atomic`                       |
...
| `CON-002` | **Resolve a summary-only Consequence.** A facilitator commits public prose with zero effects. Final lifecycle/history and one receipt commit without an artificial state change.                                                                                                             | P1       | `HTTP`, `DB`, `AUDIT`; `happy`, `consequence`, `history`                        |
...
| `LFC-003` | **Archive an entity into read-only history.** An author archives an entity. It is excluded from new setup/live references and rejects mutation, while existing profiles, statuses, actions, and receipts retain their historical subject.                                | P1       | `HTTP`, `DB`, `AUDIT`; `happy`, `archive`, `history`       |
...
| `CHF-V01` | **Block active requirement-set changes during unfinished play.** Adding/removing active field IDs while a problem is draft/open/adjudicating is rejected; field revision, readiness, and the in-flight problem remain unchanged. | P0       | `HTTP`, `DB`; `negative`, `profile`, `atomic`                             |
...
| `CCY-V03` | **Reject a stale state/rules sheet save.** A direct setup save built from an old entity-state or graph revision is rejected and preserves the authoritative state and calculated values.                                                         | P0       | `HTTP`, `DB`; `conflict`, `rules`, `atomic`                                   |
