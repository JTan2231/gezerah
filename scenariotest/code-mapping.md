...
| `WRL-002` reopen an admitted world in the appropriate library | `BuildLibrary`; `PlayLibrary`; `worlds.go#handleListWorlds`                                                      | `ui-partial`: visible library use exists, but persistence/reopen by role is not an exact journey             |
...
| `MEC-V01` invalid bounds/step                                                  | `rules.ValidateMechanicDefinition`, `ValidateStateValue`; mechanic form field errors                                                                                                                   | `lower-layer-covered`; no visible atomic rejection journey                                                                            |
...
| `CHF-V01` block schema change during unfinished play                           | `handlePutWorldCharacterFields`; active-interaction query/guard                                                                                                                                        | `observer-covered` by a `409` in `play.spec.ts`; visible feedback is a gap                                                            |
...
| `INV-005` revoke without ejecting member                                | `PeopleWorkspace`; revoke handler; membership separate from invite                                    | `observer-covered` for closed link; admitted-member continuity is not explicit |
...
| `PLY-V04` reject late submit/withdraw                            | action handlers and interaction locks/status checks                                                                                              | `uncovered`                                                                                                       |
...
| `CON-001`, `CON-002` preview/resolve summary-only                | `WorldPlay#RulingEditor`; preview/resolve handlers with empty effects                                                                            | `uncovered`                                                                                                       |
...
| `AUT-V03` omit restricted profile definitions/values                 | profile access/response loaders                                                                    | `observer-covered` for selected profile JSON; exact omission contract should be centralized |
...
| `CCY-V03` stale state/rules sheet save                               | state/rules root locks; `EntityDetail#EntitySheet`                                                 | `observer-covered` for stale rules; visible reload/recovery is a gap                        |
...
| `LFC-003` archive entity into read-only history                      | `entities.go#Server.handleArchiveWorldEntity`; historical FKs; no current frontend archive control | `uncovered`                                                                                 |
...
| `NAV-V04` no blank/false success on command failure    | command error state in feature editors; `api#ApiError`          | `uncovered` systematically                                                                  |
