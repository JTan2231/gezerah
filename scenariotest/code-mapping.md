...
| ------ | ----------------------------------------- | ---------------------------------------------------------------------- |
| `IDN`  | Identity and application entry            | Password accounts, server sessions, account controls, root choice      |
...
| Browser transport     | `web/frontend/src/api/client.ts#api`, `#worldPath`, `#worldInvitePath`                                                                                                                           | JSON calls, cookie sessions, CSRF attachment, and error mapping                             |
...
| API cross-cutting     | `internal/app/auth.go#Server.withAuthentication`, `#Server.withAuthenticatedMutation`; `support.go#requireActiveWorldMember`, `#requireWorldEditor`, `#requireWorldOwner`, `#requireFacilitator` | Server sessions, CSRF, membership, and role enforcement                                     |
...
| Persistence           | `internal/migrations/001_worldwright.sql`, `002_rules_graph_statuses.sql`, `004_password_auth.sql`                                                                                               | Accounts/sessions plus normalized world-scoped schema, revisions, immutable history         |
...
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| Frontend          | `App.tsx#App`; `features/HomeChoice.tsx#HomeChoice`; `features/IdentityGate.tsx#IdentityGate`; `features/AccountControls.tsx#AccountControls`; `api/client.ts#api`; `api/types.ts#AuthenticatedSession`                                                           | `IDN-*`, with home choice also shared by `NAV-*`                        |
| Backend           | `POST /api/auth/signup`, `POST /api/auth/signin`, `GET /api/me`, `PUT /api/me/password`, `POST /api/auth/logout`, `POST /api/auth/logout-all`; `auth.go#Server.withAuthentication`, `#Server.withAuthenticatedMutation`                                           | `IDN-*`, identity failures in `AUT-*`                                   |
| Persistence       | `004_password_auth.sql#users`, `#auth_sessions`; salted Argon2id password hashes and digest-only opaque sessions                                                                                                                                                  | `IDN-*`                                                                 |
| Existing evidence | `test/specs/scenarios/lifecycle-spine.spec.ts` creates four accounts through the rendered UI; `authentication.ui.spec.ts` proves sign-in/reload/password/session controls; `authentication.contract.spec.ts` owns server-only matrices and password-race evidence | `journey-covered`, `ui-boundary-covered`, and `direct-contract-covered` |
...
`IDN-002` is retired rather than reused. `IDN-003` is version 2 for destination
recovery across authentication. Each fixture actor owns an isolated cookie jar
and CSRF token; a UUID is retained only as assertion data and never selects the
request actor.
...
| ------ | ---------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `IDN`  | neutral entry, rendered account creation, and invite destination continuation                  | sign-in/password/session controls in `ui-boundary`; invalid credentials/session/CSRF/persistence and password-race contracts in `direct-contract`                     |
...
| ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `IDN-001` neutral area choice                                 | `HomeChoice#HomeChoice`; `App#App`; `worldRoutes#readLocation`                                                                         | `journey-covered`: the owner chooses Build before rendered signup                                                                            |
| `IDN-003` resume requested destination, version 2             | `App#App`; authenticated invite/world branches in `worldRoutes#readLocation`; lifecycle invite redemption                              | `journey-covered`: three isolated visitors retain their invite URL through rendered signup                                                   |
| `IDN-004` create password account                             | `IdentityGate#IdentityGate`; `auth.go#handleSignup`; `004_password_auth.sql`                                                           | `journey-covered`: owner/editor/player/spectator create their accounts through the rendered gate                                             |
| `IDN-005..008` sign in and manage credentials/sessions        | `IdentityGate`; `AccountControls`; signin/me/password/logout/logout-all handlers; `test/specs/ui-boundaries/authentication.ui.spec.ts` | `ui-boundary-covered`: destination, reload persistence, confirmed password rotation, current-session revocation, and account-wide revocation |
| `IDN-V01..V05`, `AUT-V07` authentication rejection/security   | `auth.go`; `004_password_auth.sql`; `test/specs/contracts/authentication.contract.spec.ts`                                             | `direct-contract-covered`: exact signup, credential, session, persistence, CSRF, and forged-header matrices                                  |
...
| `INV-003` authenticated preview/redeem into matching area               | `InvitePage`; `worldRoutes#inviteURL`; authenticated preview/redeem handlers; lifecycle rendered signup                                        | `journey-covered`: invitation metadata appears only after signup and redemption opens the offered area                                                        |
...
| `INV-V01` invalid/revoked/expired link closes without membership change | authenticated preview/redeem handlers; token hash/expiry/revocation queries; authorization matrix contract                                     | `direct-contract-covered`: every named case runs as an authenticated candidate and proves no membership/accounting change                                     |
...
| `AUT-V07` forged identity header cannot authenticate/override        | `auth.go#Server.withAuthentication`; `authentication.contract.spec.ts#AUT-V07`                                                                                                                            | `direct-contract-covered`: anonymous forgery stays `401`; an authenticated header cannot replace the session actor                                    |
...
| ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Playable world                        | `JRN-001`; `IDN-001`, `IDN-004`, `WRL-001..002`, `MEC-001..003`, `MEC-005`, `CHF-002`, `RST-001..002`                       | the spine creates an authenticated owner and the complete playable world through the UI                                                                                          |
...
| ------------------------------------------ | ------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GLO-001` UI-only mutation                 | Lifecycle actors create sessions and all durable prerequisites through rendered controls          | journey imports and capabilities make request/DB mutation unavailable; a static boundary check rejects escape hatches                               |
...
| `GLO-012` diagnostic evidence              | traces/screenshots/video on failure; `app-server.log`; centralized redaction tests                | evidence omits passwords, hashes, Cookie/Set-Cookie, CSRF/session tokens, invite tokens, and sensitive prose while retaining useful metadata        |

## 7. Existing evidence index

The required browser inventory is fourteen Playwright files containing fourteen
tests: one serial lifecycle spine, five focused UI-boundary tests, and eight
...
| `internal/app/auth.go`, `worlds.go`                                                                       | `IDN`, `WRL`, `INV`, `AUT`, `CCY`, `LFC`                                                    |
