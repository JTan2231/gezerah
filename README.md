...
Scryer is a collaborative world editor and live play surface. Authors define
...
| ------------------- | ----------------------------------------------- | ------------------------------------------------------------------------------- |
| `SCRYER_ADDR`          | `:8080`                                         | HTTP listen address.                                                            |
| `PORT`              | unset                                           | Hosting fallback when `SCRYER_ADDR` is unset.                                      |
| `SCRYER_DATABASE_URL`  | `postgres://localhost:5432/scryer?sslmode=disable` | PostgreSQL connection URL.                                                      |
| `DATABASE_URL`      | unset                                           | Hosting fallback when `SCRYER_DATABASE_URL` is unset.                              |
| `SCRYER_LOG_LEVEL`     | `info`                                          | `debug`, `info`, `warn`, or `error`.                                            |
| `SCRYER_PUBLIC_ORIGIN` | request origin                                  | Exact browser origin for authenticated writes; HTTP is loopback-only.            |
...
When `SCRYER_TEST_DATABASE_URL` is set, the backend target also starts the built
...
<https://scryingorb.com>. It is an operational
...
ignored `.scryer/deployments/<deployment-id>.json` without credentials, cookies,
