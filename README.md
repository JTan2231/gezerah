...
> [!IMPORTANT]
> **WebMCP Challenge judges:** Wrought is a pre-existing product; only the
...
Wrought is a collaborative world editor and live play surface. Authors define
...
marker is authoritative for maintained project terminology; discover and read
it through the installed `semantics.repository.explore` contract. The comprehensive system
...
Wrought is under active development. The hosted Railway instance is a
...
Actions in ChatGPT while Wrought remains the durable Play record. Accounts use a
...
| ----------------------- | --------------------------------------------------- | --------------------------------------------------------------------- |
| `WROUGHT_ADDR`          | `:8080`                                             | HTTP listen address.                                                  |
| `PORT`                  | unset                                               | Hosting fallback when `WROUGHT_ADDR` is unset.                        |
| `WROUGHT_DATABASE_URL`  | `postgres://localhost:5432/wrought?sslmode=disable` | PostgreSQL connection URL.                                            |
| `DATABASE_URL`          | unset                                               | Hosting fallback when `WROUGHT_DATABASE_URL` is unset.                |
...
When `WROUGHT_TEST_DATABASE_URL` is set, the backend target also starts the built
...
boundary. It does not create a session or prove secure-cookie issuance.

Passing deploy evidence sets `schemaVersion: 2`, records `releaseStage` as
`pre-dns` or `post-cutover`, and includes the required 30-second Railway health
timeout. It is written to
ignored `.wrought/deployments/<deployment-id>.json` without credentials, cookies,
...
without uploading source or running CI. `--pre-dns` is deploy-only and always
omits the browser probe; `--no-browser` explicitly omits it post-cutover, while
deploy mode alone accepts `--skip-ci`. The script expects already-created and
configured Railway infrastructure; it does not create a project, database,
volume, domain, or variables, and it does not automatically roll back a failed
release.
