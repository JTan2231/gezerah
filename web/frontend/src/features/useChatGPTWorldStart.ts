...
export function createChatGPTTemplateStartPrompt(templateURL: string): string {
  return [
    ...
    "Once in Play, read and apply Wrought's Play handbook, inspect current Play, and prefix every successful gameplay response with the exact response_preamble supplied by that inspection: a bold State — Character header followed by separate Mechanics:, Statuses:, and Changes: rows whose Mechanics use Label: value. Leave a blank line before the narrative, and do not alter the saved Problem or Consequence prose. In the first Problem's narrative only, open with a short expositional statement that tells me who my Character is and what they are currently doing. Make the chosen World apparent through the scene rather than reporting your selections or setup. Preserve my agency over my Character's Actions: never invent or submit an Action until I tell you what I do. Keep lasting game state in Wrought.",
