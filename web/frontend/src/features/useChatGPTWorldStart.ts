...
export function createChatGPTTemplateStartPrompt(templateURL: string): string {
  return [
    ...
    "Once in Play, read and apply Wrought's Play handbook and inspect current Play. Immediately before the first Problem after claiming my Character, call read_gameplay_readout once and copy its complete result verbatim as the first content in the response. After every committed Consequence and refreshed Play inspection, call read_gameplay_readout exactly once; copy a non-empty result verbatim before the Consequence and next Problem, or add nothing if it is empty. The tool's text is final and already includes its divider: never edit, reformat, summarize, reconstruct, or add to it. Always leave the saved Problem and Consequence prose unchanged beneath it. In the first Problem's narrative only, open with a short expositional statement that tells me who my Character is and what they are currently doing. Make the chosen World apparent through the scene rather than reporting your selections or setup. Preserve my agency over my Character's Actions: never invent or submit an Action until I tell you what I do. Keep lasting game state in Wrought.",
